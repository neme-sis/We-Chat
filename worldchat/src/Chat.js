  ...
  return (
    <div>
        ...
        <form onSubmit={send} className='form'>
            <input type="text" name="" id="" ref={sendText} placeholder="Send Text..." required
            onInvalid={invalid} onInput = {notInvalid}/>
...
const ChatMessage=({message})=>{
    ...
    const textClass= firebase.auth().currentUser.uid === uid?'sent':'recieved'
    return <div className={`chat ${textClass}`}>
        <section className='user-details'>
            <img src={photoURL} alt="" />
            {textClass === 'recieved'? <p className='recieve-name'>{displayName}</p>:<p className='recieve-name'>You</p> }
        </section>
